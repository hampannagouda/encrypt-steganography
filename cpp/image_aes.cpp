...
int main(int argc, char** argv) {
    ...
    if (argc < 5) {
        std::cout << "Usage:\n"
                  << argv[0] << " encrypt <in> <out> <password>\n"
                  << argv[0] << " decrypt <in> <out> <password>\n";
        return 1;
    }
    ...
    double sizeMB = getFileSizeMB(in);

    auto start = std::chrono::high_resolution_clock::now();

    if (mode == "encrypt")
        encryptFile(in, out, pass);
    else
        decryptFile(in, out, pass);

    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> t = end - start;

    std::cout << "\n---- Benchmark ----\n";
    std::cout << "File size : " << sizeMB << " MB\n";
    std::cout << "Time      : " << t.count() << " seconds\n";
    std::cout << "Speed     : " << (sizeMB / t.count()) << " MB/s\n";
